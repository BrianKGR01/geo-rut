# ROADMAP — v1 (archivado)

> **Archivado el 2026-09-21.** Esta fue la hoja de ruta del MVP local (sin login, sin backend,
> una sola ruta a la vez en `localStorage`). Quedó **completa** (Fases 0–4.2). El trabajo activo
> sigue en [`docs/ROADMAP.md`](./ROADMAP.md), la v2 (login, roles, Supabase, pedidos con foto).
> Este archivo se conserva como referencia histórica; no se edita más.
